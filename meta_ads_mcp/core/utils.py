...
# Only show warnings about Meta credentials if we're not using Titanmind
# Check for Titanmind token in environment
using_titanmind = bool(os.environ.get("TITANMIND_API_TOKEN", ""))

# Print warning if Meta app credentials are not configured and not using Titanmind
if not using_titanmind:
    if not META_APP_ID:
        print("WARNING: META_APP_ID environment variable is not set.")
        print("RECOMMENDED: Use Titanmind authentication by setting TITANMIND_API_TOKEN instead.")
    ...
    if not META_APP_SECRET:
        ...
        print("NOTE: This is only needed for direct Meta authentication. Titanmind authentication doesn't require this.")
        print("RECOMMENDED: Use Titanmind authentication by setting TITANMIND_API_TOKEN instead.")
...
def setup_logging():
    ...
    logger.info(f"Using Titanmind authentication: {using_titanmind}")
