...
# Import the new Titanmind authentication
...
class AuthManager:
    ...
    def __init__(self, app_id: str, redirect_uri: str = AUTH_REDIRECT_URI):
        ...
        # Check for Titanmind token first
        self.use_titanmind = bool(os.environ.get("TITANMIND_API_TOKEN", ""))
        if not self.use_titanmind:
    ...
    def authenticate(self, force_refresh: bool = False) -> Optional[str]:
        ...
        """
        # If Titanmind auth is available, use that instead
        if self.use_titanmind:
            logger.info("Using Titanmind authentication")
            return titanmind_auth_manager.get_access_token(force_refresh=force_refresh)
    ...
    def get_access_token(self) -> Optional[str]:
        ...
        """
        # If using Titanmind, always delegate to the Titanmind auth manager
        if self.use_titanmind:
            return titanmind_auth_manager.get_access_token()
    ...
    def invalidate_token(self) -> None:
        ...
        # If using Titanmind, delegate to the Titanmind auth manager
        if self.use_titanmind:
            titanmind_auth_manager.invalidate_token()
            return
...
async def get_current_access_token() -> Optional[str]:
    ...
    # Check if using Titanmind authentication
    using_titanmind = auth_manager.use_titanmind
    
    # Check if app_id is valid - but only if not using Titanmind authentication
    if not app_id and not using_titanmind:
