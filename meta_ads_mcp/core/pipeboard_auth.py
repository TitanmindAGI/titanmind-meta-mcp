"""Authentication with Meta Ads API via platform.titanmindhq.com."""
...
# Base URL for titanmind API
TITANMIND_API_BASE = "https://platform.titanmindhq.com/api"

# Debug message about API base URL
logger.info(f"Titanmind API base URL: {TITANMIND_API_BASE}")
...
class TitanmindAuthManager:
    """Manages authentication with Meta APIs via platform.titanmindhq.com"""
    def __init__(self):
        self.api_token = os.environ.get("TITANMIND_API_TOKEN", "")
        logger.debug(f"TitanmindAuthManager initialized with API token: {self.api_token[:5]}..." if self.api_token else "No API token")
        if self.api_token:
            logger.info("Titanmind authentication enabled. Will use platform.titanmindhq.com for Meta authentication.")
        else:
            logger.info("Titanmind authentication not enabled. Set TITANMIND_API_TOKEN environment variable to enable.")
    ...
    def _get_token_cache_path(self) -> Path:
        ...
        cache_path = cache_dir / "titanmind_token_cache.json"
    ...
    def initiate_auth_flow(self) -> Dict[str, str]:
        """
        Initiate the Meta OAuth flow via platform.titanmindhq.com
        
        Returns:
            Dict with loginUrl and status info
        """
        if not self.api_token:
            logger.error("No TITANMIND_API_TOKEN environment variable set")
            raise ValueError("No TITANMIND_API_TOKEN environment variable set")
            
        # Exactly match the format used in meta_auth_test.sh
        url = f"{TITANMIND_API_BASE}/meta/auth?api_token={self.api_token}"
        headers = {
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                if response.status_code == 404:
                    raise ValueError(f"Titanmind API endpoint not found. Check if the server is running at {TITANMIND_API_BASE}")
                elif response.status_code == 401:
                    raise ValueError(f"Unauthorized: Invalid API token. Check your TITANMIND_API_TOKEN.")
            ...
            logger.error(f"Connection error to Titanmind: {e}")
            logger.debug(f"Attempting to connect to: {TITANMIND_API_BASE}")
            raise
        except requests.exceptions.Timeout as e:
            logger.error(f"Timeout connecting to Titanmind: {e}")
            raise
    ...
    def get_access_token(self, force_refresh: bool = False) -> Optional[str]:
        ...
        if not self.api_token:
            logger.error("TOKEN VALIDATION FAILED: No Titanmind API token configured")
            logger.error("Please set TITANMIND_API_TOKEN environment variable")
            return None
        ...
        # If force refresh or no token/expired token, get a new one from Titanmind
        try:
            # Make a request to get the token, using the same URL format as initiate_auth_flow
            url = f"{TITANMIND_API_BASE}/meta/token?api_token={self.api_token}"
            headers = {
                "Content-Type": "application/json"
            }
            ...
            try:
                ...
                logger.error("TOKEN VALIDATION FAILED: Timeout while connecting to Titanmind API")
                logger.error(f"Could not connect to {TITANMIND_API_BASE} within 10 seconds")
                return None
            except requests.exceptions.ConnectionError:
                logger.error("TOKEN VALIDATION FAILED: Connection error with Titanmind API")
                logger.error(f"Could not connect to {TITANMIND_API_BASE} - check if service is running")
                return None
            ...
            if response.status_code != 200:
                ...
                if response.status_code == 401:
                    logger.error("Authentication failed: Invalid Titanmind API token")
                elif response.status_code == 404:
                    logger.error("Endpoint not found: Check if Titanmind API service is running correctly")
                elif response.status_code == 400:
                    logger.error("Bad request: The request to Titanmind API was malformed")
            ...
            try:
                ...
                logger.error("TOKEN VALIDATION FAILED: Invalid JSON response from Titanmind API")
                logger.error(f"Response content (first 100 chars): {response.text[:100]}")
                return None
            
            # Validate response data
            if "access_token" not in data:
                logger.error("TOKEN VALIDATION FAILED: No access_token in Titanmind API response")
            ...
            if status_code == 401:
                logger.error(f"Unauthorized: Check your TITANMIND_API_TOKEN. Response: {response_text}")
...
titanmind_auth_manager = TitanmindAuthManager() 