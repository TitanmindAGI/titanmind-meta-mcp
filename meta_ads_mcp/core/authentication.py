...
from .titanmind_auth import titanmind_auth_manager
...
async def get_login_link(access_token: str = None) -> str:
    ...
    If using Titanmind authentication (recommended), set the TITANMIND_API_TOKEN
    environment variable instead (token obtainable via https://platform.titanmindhq.com).
    ...
    """
    # Check if we're using titanmind authentication
    using_titanmind = bool(os.environ.get("TITANMIND_API_TOKEN", ""))
    
    if using_titanmind:
        # Handle Titanmind authentication
        # Check if we have a cached token
        cached_token = titanmind_auth_manager.get_access_token()
        ...
        if cached_token and not access_token:
            logger.info("get_login_link called with existing valid Titanmind token")
            return json.dumps({
                "message": "Already authenticated with Titanmind",
                "token_status": token_status,
                "token_preview": cached_token[:10] + "..." if cached_token else None,
                "authentication_method": "titanmind"
            }, indent=2)
        
        # Initiate the auth flow via Titanmind
        try:
            auth_data = titanmind_auth_manager.initiate_auth_flow()
            ...
            response = {
                "login_url": login_url,
                "token_status": token_status,
                "markdown_link": f"[Click here to authenticate with Meta Ads via Titanmind]({login_url})",
            ...
            logger.error(f"Error initiating Titanmind auth flow: {e}")
            return json.dumps({
                "error": f"Failed to initiate Titanmind authentication: {str(e)}",
                "message": "Please check your TITANMIND_API_TOKEN environment variable.",
                "authentication_method": "titanmind"
            }, indent=2)
    else:
        ...
        response = {
            ...
            "token_exchange_message": f"Your authentication token will be valid for approximately {token_duration}." + 
                                    ...
                                    " For direct Meta authentication, long-lived tokens require META_APP_SECRET. Consider using Titanmind authentication instead (60-day tokens by default)."),
            "note": "After authenticating, the token will be automatically saved."
        }
