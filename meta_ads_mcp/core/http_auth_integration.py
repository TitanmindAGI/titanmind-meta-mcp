...
class FastMCPAuthIntegration:
    ...
    def extract_token_from_headers(headers: dict) -> Optional[str]:
        ...
        # Check for Titanmind token (legacy support, to be removed)
        titanmind_token = headers.get('X-TITANMIND-API-TOKEN') or headers.get('x-titanmind-api-token')
        if titanmind_token:
            logger.debug("Found Titanmind token in legacy headers")
            return titanmind_token
