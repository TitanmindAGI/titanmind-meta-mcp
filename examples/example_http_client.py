...
class MetaAdsMCPClient:
    ...
    def __init__(self, base_url: str = "http://localhost:8080", 
                 titanmind_token: Optional[str] = None,
                 meta_access_token: Optional[str] = None):
        """Initialize the client
        
        Args:
            base_url: Base URL of the MCP server
            titanmind_token: Titanmind API token (recommended)
        ...
        # Add authentication
        if titanmind_token:
            self.headers["Authorization"] = f"Bearer {titanmind_token}"
            print(f"✅ Using Titanmind authentication")
...
def main():
    ...
    titanmind_token = os.environ.get("TITANMIND_API_TOKEN")
    meta_token = os.environ.get("META_ACCESS_TOKEN")
    
    if not titanmind_token and not meta_token:
        print("⚠️  No authentication tokens found in environment")
        print("   Set TITANMIND_API_TOKEN or META_ACCESS_TOKEN for full functionality")
        print("   Using test token for demonstration...")
        titanmind_token = "demo_token_12345"
    
    # Create client
    client = MetaAdsMCPClient(
        titanmind_token=titanmind_token,
    ...
    try:
        ...
        if "error" in parsed_content and "Authentication Required" in parsed_content["error"]["message"]:
            ...
            print(f"   Use a real Titanmind token for actual data access.")
        else:
    ...
    print("\n💡 Next steps:")
    print("   1. Set TITANMIND_API_TOKEN environment variable")
