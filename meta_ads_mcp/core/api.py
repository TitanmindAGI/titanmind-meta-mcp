    ...
    async def wrapper(*args, **kwargs):
        try:
            ...
            if 'access_token' not in kwargs or not kwargs['access_token']:
                try:
                    ...
                    if access_token:
                        ...
                        if (auth_manager.app_id == "YOUR_META_APP_ID" or not auth_manager.app_id) and not auth_manager.use_titanmind:
                        ...
                        elif auth_manager.use_titanmind:
                            logger.error("TOKEN VALIDATION FAILED: Titanmind authentication enabled but no valid token available")
                            logger.error("Complete authentication via Titanmind service or check TITANMIND_API_TOKEN")
                        else:
            ...
            if 'access_token' not in kwargs or not kwargs['access_token']:
                ...
                app_id = auth_manager.app_id
                using_titanmind = auth_manager.use_titanmind
                
                logger.error("TOKEN VALIDATION SUMMARY:")
                logger.error(f"- Current app_id: '{app_id}'")
                logger.error(f"- Environment META_APP_ID: '{os.environ.get('META_APP_ID', 'Not set')}'")
                logger.error(f"- Titanmind API token configured: {'Yes' if os.environ.get('TITANMIND_API_TOKEN') else 'No'}")
                logger.error(f"- Using Titanmind authentication: {'Yes' if using_titanmind else 'No'}")
                
                # Check for common configuration issues - but only if not using Titanmind
                if not using_titanmind and (app_id == "YOUR_META_APP_ID" or not app_id):
                ...
                elif using_titanmind:
                    logger.error("ISSUE DETECTED: Titanmind authentication configured but no valid token available")
                    logger.error("ACTION REQUIRED: Complete authentication via Titanmind service")
                    ...
                    "error": {
                        ...
                        "details": {
                            ...
                            "configuration_status": {
                                ...
                                "titanmind_enabled": bool(os.environ.get('TITANMIND_API_TOKEN')),
                            },
